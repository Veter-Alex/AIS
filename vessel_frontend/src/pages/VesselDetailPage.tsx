...
const VesselDetailPage: React.FC = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: VesselUpdate) => 
      vesselApi.updateVessel(imo!, data),
    onSuccess: () => {
  ...
  const handleSave = () => {
    ...
    if (editedVessel.call_sign !== vessel?.call_sign) updates.call_sign = editedVessel.call_sign;
    if (editedVessel.general_type !== vessel?.general_type) updates.general_type = editedVessel.general_type;
    if (editedVessel.detailed_type !== vessel?.detailed_type) updates.detailed_type = editedVessel.detailed_type;
    if (editedVessel.flag !== vessel?.flag) updates.flag = editedVessel.flag;
    if (editedVessel.year_built !== vessel?.year_built) updates.year_built = editedVessel.year_built;
    if (editedVessel.length !== vessel?.length) updates.length = editedVessel.length;
    if (editedVessel.width !== vessel?.width) updates.width = editedVessel.width;
    if (editedVessel.dwt !== vessel?.dwt) updates.dwt = editedVessel.dwt;
    if (editedVessel.gt !== vessel?.gt) updates.gt = editedVessel.gt;
    if (editedVessel.home_port !== vessel?.home_port) updates.home_port = editedVessel.home_port || null;
    if (editedVessel.description !== vessel?.description) updates.description = editedVessel.description || null;
  ...
  const renderField = (label: string, key: keyof Vessel, type: 'text' | 'number' = 'text') => {
    const value = vessel[key];
    const displayValue = value ?? '-';
    
    if (isEditing && key !== 'info_source' && key !== 'updated_at' && key !== 'id' && key !== 'photo_path') {
      if (type === 'number') {
        return (
          <div key={label} className="border-b border-dark-border pb-3">
            ...
            <input
              ...
              onChange={(e) => setEditedVessel({ ...editedVessel, [key]: e.target.value ? parseInt(e.target.value) : null })}
      ...
      return (
        <div key={label} className="border-b border-dark-border pb-3">
          ...
          <input
            ...
            onChange={(e) => setEditedVessel({ ...editedVessel, [key]: e.target.value })}
            className="w-full px-3 py-2 bg-dark-bg border border-dark-border rounded text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
      );
    ...
    <div className="min-h-screen bg-dark-bg">
      ...
      <header className="bg-dark-card border-b border-dark-border">
        <div className="container mx-auto px-4 py-6">
          ...
          <p className="text-gray-400 mt-2">IMO: {vessel.imo} • MMSI: {vessel.mmsi}</p>
        </div>
      </header>
            ...
            <div className="bg-dark-card border border-dark-border rounded-lg overflow-hidden">
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-semibold text-gray-100">Характеристики судна</h2>
                  {!isEditing ? (
                    <button
                      onClick={handleEdit}
                ...
                {updateMutation.isError && (
                  <div className="mb-4 p-3 bg-red-900/20 border border-red-500 rounded text-red-400 text-sm">
                    Ошибка при сохранении: {updateMutation.error instanceof Error ? updateMutation.error.message : 'Неизвестная ошибка'}
                  </div>
                )}
                ...
                <div className="mt-6 pt-6 border-t border-dark-border">
                  <h3 className="text-xl font-semibold text-gray-100 mb-3">Описание</h3>
                  {isEditing ? (
                    <textarea
                      value={editedVessel.description as string || ''}
                      onChange={(e) => setEditedVessel({ ...editedVessel, description: e.target.value })}
                    ...
                    <div>
                      {vessel.description ? (
                        <p className="text-gray-300 leading-relaxed whitespace-pre-wrap">{vessel.description}</p>
                      ) : (
