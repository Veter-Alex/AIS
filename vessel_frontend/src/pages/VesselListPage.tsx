...
// Главная страница каталога судов.
//
// Страница объединяет:
// - фильтрацию и поиск;
...
const VesselListPage: React.FC = () => {
  ...
  const handleSearch = () => {
    // Явный submit кнопкой/Enter использует ту же нормализацию, что и debounce.
    const trimmedSearch = searchInput.trim();
    setFilters({ ...filters, search: trimmedSearch || undefined, page: 1 });
  };

  const handleSort = (field: string) => {
    // Повторный клик по тому же полю меняет направление сортировки.
    setFilters({
      ...filters,
      sort_by: field,
      sort_order:
  ...
  const totalPages = vesselsData
    ? Math.ceil(vesselsData.total / vesselsData.per_page)
    : 0;

  // Загрузка источников
  const { data: sourcesData } = useQuery({
    queryKey: ["sources"],
    queryFn: vesselApi.getSources,
  });

  const vesselTypes = stats?.vessel_types.map((vt) => vt.general_type) || [];
  const flags = stats?.flags.map((f) => f.flag) || [];
  const sources = sourcesData?.sources.map((s) => s.info_source) || [];
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {vesselsData && (
                <span className="text-gray-400">
                  ...
                  {Math.min(
                    vesselsData.page * vesselsData.per_page,
                    vesselsData.total,
                  )}{" "}
                  из {vesselsData.total}
                </span>
              )}
            </div>
